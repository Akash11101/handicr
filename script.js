...
document.addEventListener('DOMContentLoaded', () => {
        ...
        _products: [
             {
                ...
                image: [
                    'biking.jpg',
                    'https://via.placeholder.com/600x600.png?text=Scarf+Detail',
                    'https://via.placeholder.com/600x600.png?text=Scarf+Texture',
                    'https://via.placeholder.com/600x600.png?text=Scarf+On+Model'
                ],
            ...
            },
            {
                ...
                // This product still has a single image string, the code will handle it
    ...
    const renderProductCard = (product) => {
        ...
        // Use the first image for the card, whether it's an array or string
...
if (e.target.matches('.buy-now')) {
    ...
    async function initProductDetailPage() {
        ...
        if (product) {
            ...
            const thumbnailsHtml = images.length > 1 ? `
                <div class="thumbnail-container">
                    ${images.map((img, index) => `
                        <img src="${img}" alt="${product.name} thumbnail ${index + 1}" class="thumbnail-image ${index === 0 ? 'active' : ''}" data-full-image="${img}">
                    `).join('')}
                </div>` : '';
                    ...
                    <div class="product-detail-layout">
                        <div class="product-image-gallery">
                             <div class="main-image-container">
                                <img src="${mainImageUrl}" alt="${product.name}" class="main-product-image" id="main-product-image">
                            </div>
                            ${thumbnailsHtml}
                        </div>
            ...
            // Add event listener for thumbnails after rendering
            const thumbnailContainer = ui.productDetailContainer.querySelector('.thumbnail-container');
            if (thumbnailContainer) {
                thumbnailContainer.addEventListener('click', (e) => {
                    if (e.target.matches('.thumbnail-image')) {
                        const mainProductImage = document.getElementById('main-product-image');
                        mainProductImage.src = e.target.dataset.fullImage;
                        
                        thumbnailContainer.querySelector('.thumbnail-image.active').classList.remove('active');
            ...
            // Naming conflict fix: related products should go in a different container
            const relatedProductsGrid = document.getElementById('related-products-grid'); 
            if(relatedProductsGrid) {
                relatedProductsGrid.innerHTML = related.map(renderProductCard).join('');
            }
    ...
    async function initArticlePage() {
        ...
        if (post) {
            ...
            if (ui.relatedArticlesGrid) {
                const allPosts = await mockApi.getBlogPosts();
                const relatedPosts = allPosts.filter(p => p.id !== postId).slice(0, 3);
                if (relatedPosts.length > 0) {
                    ui.relatedArticlesGrid.innerHTML = relatedPosts.map(renderBlogPostCard).join('');
                } else {
                    const relatedSection = ui.relatedArticlesGrid.closest('.related-articles-section');
